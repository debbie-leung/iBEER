...
```
```{r}
m_weight_rate <- aov(my_data$weight_loss_percent ~ my_data$heating_rate)
summary(m_weight_rate)
TukeyHSD(m_weight_rate)
```
```{r}
m_weight_time <- aov(my_data$weight_loss_percent ~ my_data$time_point)
summary(m_weight_time)
TukeyHSD(m_weight_time)
```
...
```
```{r}
m_hr_int <- aov(my_data$heart_rate ~ my_data$heating_rate*my_data$tidal_height, data = my_data)
TukeyHSD(m_hr_int)
```
